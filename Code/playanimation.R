...
example.play <- team |>
  ...
  group_by(team) |>
  mutate(side = mean(x[event=='ball_snap'])) |>
  ungroup() |>
...
for(i in seq_along(frames)){
  ...
  dchull <- chull_plot(df=example.play, frames[i])       #Gets area of convex hull
  dcentroid <- chull_plot_centroid(df=example.play, frames[i])
  ballpos <- ball_position1(df=example.play, frames[i])  #Gets position of ball
  dchull$role = NA
  dchull$jerseyNumber = NA
  dcentroid$role = NA
  dcentroid$jerseyNumber = NA
  dplayer$valx = 'player'
  dchull$valx = 'hull'
  dcentroid$valx = 'centroid'
  ballpos$valx  = 'football'
  fulldf[[i]] = rbind(dplayer,dchull,ballpos,dcentroid)
}
...
#First Down Line
fd <- ifelse(pos_side>def_side, losx - ytg, losx + ytg)
#Down and Distance
dnd <- unique(example.play$dnd)
x0 <- ifelse(pos_side>def_side, losx - 2.25, losx + 2.25)
xend0 <- ifelse(pos_side>def_side, losx + 2.125, losx - 2.125)
arrowx <- ifelse(pos_side>def_side, losx - 2, losx + 2)
adjust <- ifelse(pos_side>def_side, 0, 0.01)
yend0 <- ifelse(losy>26.665, 15.33, 38)
#Field Parameters
ydmax <- ifelse(pos_side>def_side, (round(max(playdf$x)) + 5), (losx + 20))
ydmin <- ifelse(pos_side>def_side, (losx - 20), (round(min(playdf$x)) - 5))
